...
public class ExpandableListAdapter extends BaseExpandableListAdapter {
    ...
    public ExpandableListAdapter(Activity act, SparseArray<GroupReport> groups) {
        this.activity = act;
        this.groups = groups;
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        if (convertView == null) {
            convertView = this.inflater.inflate(R.layout.list_group_rech, parent, false);
        }
        GroupReport group = (GroupReport) getGroup(groupPosition);
        ((TextView) convertView.findViewById(R.id.req)).setText(group.getRequestId());
        ((TextView) convertView.findViewById(R.id.custno)).setText(group.getCustomerNo());
        ((TextView) convertView.findViewById(R.id.amount)).setText(group.getAmount());
        ((TextView) convertView.findViewById(R.id.status)).setText(group.getStatus());
        if (groupPosition % 2 == 0) {
            convertView.setBackgroundColor(Color.parseColor("#FDFDFE"));
        } else {
